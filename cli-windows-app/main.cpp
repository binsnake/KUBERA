...
void save_cpu_state ( KUBERA& ctx, CONTEXT& context ) {
  ...
  if ( ( context.ContextFlags & CONTEXT_CONTROL ) == CONTEXT_CONTROL ) {
    context.SegSs = ctx.get_reg ( Register::SS, 2 );
    context.SegCs = ctx.get_reg ( Register::CS, 2 );
    context.Rip = ctx.get_reg ( Register::RIP );
    context.Rsp = ctx.get_reg ( Register::RSP );
    context.EFlags = ctx.get_rflags();
  }
  ...
  if ( ( context.ContextFlags & CONTEXT_SEGMENTS ) == CONTEXT_SEGMENTS ) {
    context.SegDs = ctx.get_reg ( Register::DS, 2 );
    context.SegEs = ctx.get_reg ( Register::ES, 2 );
    context.SegFs = ctx.get_reg ( Register::FS, 2 );
    context.SegGs = ctx.get_reg ( Register::GS, 2 );
  }
  ...
  if ( ( context.ContextFlags & CONTEXT_FLOATING_POINT ) == CONTEXT_FLOATING_POINT ) {
    ...
    context.FltSave.TagWord = fpu.fpu_tag_word.value;
    for ( int i = 0; i < 8; i++ ) {
...
int main ( ) {
	windows::ntdll = windows::get_module_handle ( "ntdll.dll" );
	__assume( windows::ntdll != nullptr );

	windows::ldr_initialize_thunk =
		reinterpret_cast< uint64_t >( windows::get_proc_address ( windows::ntdll, "LdrInitializeThunk" ) );
	windows::rtl_user_thread_start =
		reinterpret_cast< uint64_t >( windows::get_proc_address ( windows::ntdll, "RtlUserThreadStart" ) );
	windows::ki_user_apc_dispatcher =
		reinterpret_cast< uint64_t >( windows::get_proc_address ( windows::ntdll, "KiUserApcDispatcher" ) );
	windows::ki_user_exception_dispatcher =
		reinterpret_cast< uint64_t >( windows::get_proc_address ( windows::ntdll, "KiUserExceptionDispatcher" ) );
