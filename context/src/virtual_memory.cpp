...
namespace kubera
{
	...
	uint64_t VirtualMemory::alloc ( std::size_t size, uint8_t prot, std::size_t alignment ) {
	...
	uint64_t VirtualMemory::alloc_at ( uint64_t base_addr, std::size_t size, uint8_t prot ) {
	...
	void* VirtualMemory::translate ( uint64_t addr, uint8_t access ) {
		...
		for ( auto& e : cache ) {
			if ( e.virt == virt_page ) {
				...
				if ( !e.page->present ) {
					...
					if ( !e.page->data ) {
						if constexpr ( verbose_memory ) {
							std::println ( "Access violation at address {:#x} with access {:#x} (memory not commited)", addr, access );
						}
						return nullptr;
					}
	...
	void* VirtualMemory::translate_bypass ( uint64_t addr ) {
		uint64_t virt_page = addr & ~( page_size - 1 );
		for ( auto& e : cache ) {
			if ( e.virt == virt_page ) {
				if ( !e.page->present ) {
