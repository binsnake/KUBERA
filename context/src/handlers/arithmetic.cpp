...
void handlers::imul ( const iced::Instruction& instr, KUBERA& context ) {
	multiply<int64_t> ( instr, context, [ ] ( auto& flags, uint64_t low_res, uint64_t high_res, size_t op_size )
	{
		const int64_t sres = static_cast< int64_t >( low_res << ( 64 - op_size * 8 ) ) >> ( 64 - op_size * 8 );
		flags.CF = flags.OF = ( high_res != 0 || ( sres != static_cast< int64_t >( low_res ) ) );
		flags.SF = ( low_res >> ( op_size * 8 - 1 ) ) & 1;
		flags.ZF = ( low_res == 0 );
		flags.PF = std::popcount ( low_res & 0xFF ) % 2 == 0;
	} );
}
